...
public class OverpassMapDataDaoTest
{
	...
	{
		ElementCount count = createDao().queryCount(BBOX + "way[building];out count;");
	...
	@Test public void queryTable()
	{
		...
		createDao().queryTable("[out:csv(name, highway)]" + BBOX + "way[highway][name];out;", handler);
	...
	@Test public void queryElements()
	{
		...
		createDao().queryElements(BBOX + "way[name=\"Alter Wall\"];out meta;", new MapDataHandler()
		{
	...
	{
		createDao().queryElementsWithGeometry(BBOX + "nwr[building];out meta geom;", new MapDataWithGeometryHandler()
		{
